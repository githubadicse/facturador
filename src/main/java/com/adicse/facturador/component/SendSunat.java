...
@Component
public class SendSunat {

	static VariablesGlobales VarGlo = new VariablesGlobales();

	//  PARAMETROS PARA ENVIO DE FACTURA A LA SUNAT
	...
	public void EnviarSunat(String ruc, String UsuSol, String PassSol, String RutaCPE, String NombreCPE, String RutaRta,
			String NombreArchvoRta) {
		...
		try {
			...
			URL url = new URL(new URL("https://e-beta.sunat.gob.pe:443/ol-ti-itcpfegem-beta/billService"), "",
...
//            String NombreArchvoRta = "R-20209098114-03-B002-00715015";
			...
			// body.getElementsByTagName("applicationResponse").item(0).getTextContent());
			String str1 = body.getElementsByTagName("applicationResponse").item(0).getTextContent();

			VarGlo.decode(str1, RutaRta +"R-"+ NombreArchvoRta);
	...
	/*
	 * PARA CARGAR CATALOGO DE COMISIONES ========GetCommissions=========
	 */

	public String[] ConexionCPEBeta(String ruc, String UsuarioSol, String PassSol, String NombreCPE, String RutaCPE,
			String RutaCDR, String RutaXMLCDR, String RutaWS) {
		...
		try {
			...
			// URL("https://e-beta.sunat.gob.pe:443/ol-ti-itcpfegem-beta/billService"),
					= new URL(new URL(RutaWS), "", new URLStreamHandler() {
						@Override
						protected URLConnection openConnection(URL url) throws IOException {
							URL target = new URL(url.toString());
							URLConnection connection = target.openConnection();
			...
			SOAPMessage soapResponse = soapConnection.call(CPEsendBill(ruc, UsuarioSol, PassSol, NombreCPE, RutaCPE),
					url);
			...
			if (NombreCPE.substring(12, 14).equals("RA") || NombreCPE.substring(12, 14).equals("RC")) {// ==============PARA
																										// LOS DETALLES
																										// SOLO (TICKET)
				...
				if (Rta.getLength() > 0) {
					...
					Retorno[0] = "1";// todo ok
					Retorno[1] = VarGlo.valorXML(RutaCDR, "*", "ResponseCode");/// OPTENEMOS ALGUN CODIGO SUNAT DEL
																				/// CDRXML
					Retorno[2] = VarGlo.valorXML(RutaCDR, "*", "Description").toUpperCase();/// OPTENEMOS ALGUN
																							/// DESCRIPCION SUNAT DEL
																							/// CDRXML
	...
	// ======ENVIO DOCUMENTOS (BOLETA, FACTURA, NOTA CREDITO, NOTA DEBITO, ANULADOS,
	// RESUMEN BOLETA)=========================================
	private static SOAPMessage CPEsendBill(String ruc, String UsuarioSol, String PassSol, String NombreCPE,
			String RutaCPE) throws Exception {
